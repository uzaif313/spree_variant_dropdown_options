Spree::Product.class_eval do
  ...
  def variants_for_option_value(value)
    ...
    if value.is_a? String
      ...
      @_variant_option_values.select { |i| i.option_value_ids == value }
    end
  end

  def identified_variant(id_value_array)
    @_variant_option_values ||= variants.includes(:option_values).all
    @_variant_option_values.select { |i| i.option_value_ids == id_value_array }.first
  end
